...
4. Сеть сейчас строится вокруг SoftAP, а команды двигателя из UART/HTTP/WebSocket сериализуются через одну FreeRTOS queue и исполняются из app tick.
5. Browser-control embedded API intentionally same-origin; wildcard CORS больше не является архитектурным контрактом.
6. `sdkconfig` — локальный generated файл и не является tracked source of truth; tracked defaults находятся в `sdkconfig.defaults`.
...
## Почему нет ALL-IN-ONE

Раньше в каталоге лежал `ESP-P4-LAB-ALL-IN-ONE.md` — полная копия всех страниц. Он создавал второй, быстро устаревающий источник истины и почти удваивал документационный контекст для агентов.

Canonical documentation — отдельные страницы + индексы этого каталога. Если когда-нибудь понадобится единый экспорт для публикации, его следует генерировать как build artifact, а не хранить рядом с source pages.
