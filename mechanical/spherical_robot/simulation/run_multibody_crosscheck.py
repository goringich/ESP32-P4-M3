#!/usr/bin/env python3
"""One-shot multibody verification entrypoint for CI and bounded agents.

All intermediate evidence lives inside one process/sandbox lifetime so MuJoCo,
Project Chrono, and the comparison stage bind the same exact worktree revision.
"""
...
def execute(output: Path) -> tuple[int, dict[str, Any]]:
  with tempfile.TemporaryDirectory(prefix="spherical-multibody-") as tmp:
    ...
    payload = {
      **final_evidence,
      "entrypoint": {
        "schema_version": "2026-09-21.spherical-multibody-entrypoint.v1",
        "stages": stage_receipts,
