...
class ParameterContractTests(unittest.TestCase):
  def test_sizing_report_uses_current_dimension_keys(self) -> None:
    ...
    self.assertGreater(
      report["pendulum"]["design_gravity_torque_nm"],
      report["pendulum"]["gravity_torque_nm"],
    )
    self.assertEqual(
      report["pendulum"]["torque_basis"],
      "worst_configured_ballast_and_arm_radius",
    )
    self.assertFalse(
      report["actuator_screening"]["meets_continuous_torque_screen"]
    )
